...
To expose the FIB tools to external clients, an MCP server is implemented. This server registers the available tools, such as course search, and defines their input schemas. It handles incoming tool calls by dispatching them to the appropriate underlying functions. This setup adheres to the Model Context Protocol, allowing the FIB agent's capabilities to be discovered and used by other MCP-compatible AI systems.
